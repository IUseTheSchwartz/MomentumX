...
export default function NewAgentCourse() {
  ...
  async function completeVideo(index) {
    if (!sessionUserId || saving || completingRef.current || completedIndexes.has(index)) return;
    ...
    try {
      const finalWatched = Math.max(duration || 0, maxWatchedRef.current || 0);
      await saveVideoProgress(index, finalWatched, true);
    } catch (error) {
